...
// 注意: callback的运行会早于Cboard构造函数的完成
{
  ...
  auto can_interface = config["can_interface"] ? config["can_interface"].as<std::string>() : "";
  if (can_interface.empty() || if_nametoindex(can_interface.c_str()) == 0) {
    tools::logger()->info("[Cboard] CAN interface '{}' not available, using dummy IMU.", can_interface);
...
{
  if (!has_can_) return Eigen::Quaterniond::Identity();
